<?php 
  ...
  if ($_SESSION['nivel']==1){
    ...
    $queryAgTotal = "select * from agenda where idTecnico = $idTecnico and (requiere = 1 or requiere = 5 or requiere = 6 or requiere = 7) ORDER BY fecha ASC";
    $agTotal = mysql_query($queryAgTotal, $conexion1) or die(mysql_error());
    $agTotalNumero = mysql_num_rows($agTotal);

    $queryAgEst1 = "select * from agenda where idTecnico = $idTecnico and requiere = 1 ORDER BY fecha ASC";
    $agEst1 = mysql_query($queryAgEst1, $conexion1) or die(mysql_error());
    $agEst1Numero = mysql_num_rows($agEst1);

    $queryAgEst3 = "select * from agenda where idTecnico = $idTecnico and requiere = 3 ORDER BY fecha ASC";
    $agEst3 = mysql_query($queryAgEst3, $conexion1) or die(mysql_error());
    $agEst3Numero = mysql_num_rows($agEst3);

    $queryAgEst4 = "select * from agenda where idTecnico = $idTecnico and requiere = 4 ORDER BY fecha ASC";
    $agEst4 = mysql_query($queryAgEst4, $conexion1) or die(mysql_error());
    $agEst4Numero = mysql_num_rows($agEst4);

    $queryAgEst5 = "select * from agenda where  idTecnico = $idTecnico and requiere = 5 ORDER BY fecha ASC";
    $agEst5 = mysql_query($queryAgEst5, $conexion1) or die(mysql_error());
    $agEst5Numero = mysql_num_rows($agEst5);

    $queryAgEst6 = "select * from agenda where  idTecnico = $idTecnico and requiere = 6 ORDER BY fecha ASC";
    $agEst6 = mysql_query($queryAgEst6, $conexion1) or die(mysql_error());
    $agEst6Numero = mysql_num_rows($agEst6);

    $queryAgEst7 = "select * from agenda where  idTecnico = $idTecnico and requiere = 7 ORDER BY fecha ASC";
              ...
              <div id="contenedorOrdenesAdmin">           
                ...
                <script type="text/javascript" language="javascript">
                  function cargapagina(requiere){
                    window.location ="listadoOrdenesTecnico.php?req="+requiere;
                    }
                </script>  
