...
if (($_SESSION['nivel']==0) &&  ($_SESSION['id']!="") || ($_SESSION['id']==70)){
  	...
  	$idS = $_GET["idS"];
  	$sql = "SELECT * FROM edificio  WHERE idSucursal = $idS ORDER BY nombreEdificio ASC";
    ...
    <body>
     <div data-role='page' data-theme='chase' id='contacto'>
        ...
        <div class='content' data-role='content'>
          	<h2>Edificios : <?= $nombreSucursal?> - <?= $cia;?></h2>
            <ul data-inset='true' data-role='listview' data-theme='d'>
		        <li>
	                <form name="1" action="list_suc.php?idE=<?= $idE;?>" method="post">
	                    <center>
		            ...
		            <div id="contenedor">          	
		                ...
		                <?php
		                	$i=1;
		    	            while($rowEdificio=mysql_fetch_assoc($result)){
		                	...
		                	<div id="contenidos">
		    	          		...
		    	          		<div id="columna2" style="background-color:#<?php echo $color;?>">                  			
				                	<?php echo strtoupper($rowEdificio["nombreEdificio"])."&nbsp;".strtoupper($rowCliente["apellidos"]); ?>
				                </div> 
				            <? if ($_SESSION['id']!=70){?>                     
		    	          		<div id="columna3" style="background-color:#<?php echo $color;?>">                  			
				        			<form name="1" action="editEdificio.php" method="post">
										<input type="hidden" name="idEdificio" id="idEdificio" value="<?php echo $rowEdificio["idEdificio"];?>">
								        <center>
 											<button aria-label='button1' data-theme='chase' name='button1' role='button' type='submit' value='submit' >
				            ...
				            <?}?> 
		    	          		<div id="columna3" style="background-color:#<?php echo $color;?>">                  			
									<form name="2" action="listEquipment.php?idEd=<?= $rowEdificio["idEdificio"];?>" method="post">
								        <center>
 											<button aria-label='button1' data-theme='chase' name='button1' role='button' type='submit' value='submit' >
              									<span aria-hidden='true'>Equipos</span>
          									</button>
										</center>
									</form>		
				                </div> 				                                	
