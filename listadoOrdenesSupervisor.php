<?php 
  ...
  if ($_SESSION['nivel']==5){
    ...
    if ($requiere == 0){
      mysql_select_db($database_conexion1, $conexion1);
	    $queryAgenda = "select * from agenda where requiere = 1 or requiere = 3 or requiere = 6 or requiere = 7 ORDER BY idAgenda DESC";
	    $agendaArray = mysql_query($queryAgenda, $conexion1) or die(mysql_error());
      $agendaTotalNumero = mysql_num_rows($agendaArray);      
	   	}
  	else {
      mysql_select_db($database_conexion1, $conexion1);
	    $queryAgenda = "select * from agenda where requiere = $requiere  ORDER BY idAgenda DESC";
      $agendaArray = mysql_query($queryAgenda, $conexion1) or die(mysql_error());
      $agendaTotalNumero = mysql_num_rows($agendaArray);      
		  }		
    ?>
          ...
          <ul data-inset='true' data-role='listview' data-theme='d'>
            ...
            <script type="text/javascript" language="javascript">
              function cargapagina(idAg,requiere){
                if (requiere == 0){                
                  window.location ="gestionOrdenesSupervisor.php?idAg="+idAg;
                  }
                else{
                  window.location ="gestionOrdenesSupervisor2.php?idAg="+idAg;
                  }
                }
           </script>  
    				<?php
